...
We used convolutional neural networks (CNNs) to learn discriminative models for the dementia classes.  Since the 4th class (moderate dementia) had a small sample size, we compared model performance between a three class representation of dementia progression (with the two smallest classes, moderate and mild dementia, pooled together) and a four class representation of dementia progression.
...
Model 2 uses the same CNN specification as model 1, except now the class vectors are of length four and not three.
...
Sabir wrote the data read-in code, set up CUDA with R on his personal machine and used it to train all the CNN models (reducing runtime by up to 2 orders of magnitude), wrote the introduction and data read-in sections in the report and presentation, and performed much of the R Markdown formatting of the final project.
